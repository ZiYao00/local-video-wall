...
Local Video Wall 可以把一个本地文件夹转换成本地图片/视频媒体墙，可运行在 Chrome/Edge 或 Windows Desktop 宿主中，适合需要生成、下载、对比和整理大量本地 AI 图片与视频的用户。
...
- 扫描时自动忽略 0 Byte 的媒体占位文件，但不会删除源文件。
...
- 所有请求都会校验本地服务 Host；会写入数据或触发系统操作的 `POST` 请求还需要本次启动生成的会话 Token，并校验页面来源为本机服务。
- 不会将媒体文件上传到云端服务。
