...
- [ ] **T1 `service.bat` PID 守护**
  启动写 PID 文件；停止/重启读 PID 并核对进程命令行含本项目 `app.py` 路径，
  核对通过才 `taskkill`；PID 文件不存在时只报“未找到服务”，绝不按端口杀。
  - 影响：`service.bat`、可能需 `app.py` 写 PID 文件
- [ ] **T2 清理旧项目名**
